 ...
 * the fewest subarrays needed so no subarray exceeds mid — if that count is <= k the limit is
...
public class H_CanSplitMinSum {
        ...
        // low: smallest possible answer — every element must fit, so max element is the floor.
        // high: largest possible answer — one subarray with everything.
        int low=0;
        int high=0;
        ...
        // low is the first value where canSplit returns true — the minimum feasible limit.
        return low;
    }
    ...
    public boolean canSplit(int[] nums, int k, int limit){
        ...
        for(int num:nums){
            if(num+curr>limit){
                // Current element can't fit — start a new subarray with it as the first element.
                splits++;
